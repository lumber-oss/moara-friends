...
- **校验失败**：会在 Issue 中评论失败原因，Issue 关闭为 `not_planned`。**修改后请重新提交一个新 Issue**。
...
A: 失败的 Issue 会被自动关闭。请直接通过 [apply.html](./apply.html) 重新提交一个新 Issue。已处理的 Issue 不会重复审核（通过隐藏 marker 实现幂等）。
