...
修改 Issue 正文（点右上角编辑按钮，修正错误字段或补上回链），然后任选一种方式触发重新校验（**仅 Issue 创建者和仓库管理员可触发**）：
...
>
> **权限**：只有 Issue 创建者和仓库管理员（owner/admin）能触发 `/recheck`。陌生人评论 `/recheck` 会被拒绝并提示。

#### 6. 关于邮件通知

为避免频繁邮件打扰，bot 的所有状态更新（处理中 → 通过/失败）会**合并到同一条评论**：

- 首次创建状态评论时，GitHub 会发邮件通知（一次）
- 后续状态更新通过**编辑同一条评论**实现，不会发邮件
- 用户每次只收到 1 封邮件，避免「邮件轰炸」
