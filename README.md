...
- **方式一：网页表单（推荐，无需 Fork 仓库）** —— 在线填写表单 → 登录 GitHub 提交 Issue → 自动审核
- **方式二：PR**（适合熟悉 Git 的用户）—— Fork 仓库 → 编辑 JSON → 提交 PR → 自动校验合并

两条路径的校验规则完全一致（字段 / SSRF / 可达性 / 回链验证）。
...
- **校验通过**：自动写入 `data/friends/<filename>.json`，触发构建，CDN 刷新后即可在本站友链页看到。Issue 会被关闭为 `completed`。
- **校验失败**：会在 Issue 中评论失败原因，Issue 关闭为 `not_planned`。**无需新建 Issue**，按下方方式触发重新校验即可。

校验项：字段格式 / SSRF 防护 / 回链域名一致性 / URL 与头像可达性 / 回链验证（支持 Playwright 兜底动态渲染页面）。

#### 5. 校验失败后如何重新校验

修改 Issue 正文（点右上角编辑按钮，修正错误字段或补上回链），然后任选一种方式触发重新校验：

- **方式 A：重新打开 Issue** —— 在 Issue 页底部点「Reopen issue」按钮，bot 会自动重新校验
- **方式 B：评论 `/recheck`** —— 在该 Issue 评论区直接输入 `/recheck` 并提交，bot 会自动重新校验（即使 Issue 已关闭也可以用这种方式，bot 会自动帮你重新打开）

> 失败评论底部自带一个「🔄 重新校验」折叠块，展开即可看到这两种方式的说明。
>
> 已通过的 Issue 不会被重复触发（避免重复写入文件）。已失败的 Issue 可无限次重新校验，直到通过或放弃。
